...
run_sql_and_return_html(cnx,"""
SELECT 
  `Country Code`, `Short Name`, `Country Code`, `WB-2 Code`, `Region`
FROM 
  wdi_country
WHERE 
...
execute_ddl(cnx,"""
-- Example: Suppose it was "South Sudan"
UPDATE wdi_country
...
WHERE NOT EXISTS (
  SELECT 1
  FROM wdi_country wc
  WHERE wc.`Region` = r.`Region`
    AND wc.`Income Group` = i.`Income Group`
)
...
SELECT 
  ...
  b.`Income Group`,
  b.country_count,
  (b.country_count * 100.0 / t.total_countries) AS percent_of_total
...
totals AS (
  SELECT SUM(country_count) AS total FROM counts
)
SELECT 
  `Region`,
  ROUND(SUM(CASE WHEN `Income Group` = 'High income' THEN country_count END) * 100.0 / total, 2) AS High_Income,
  ROUND(SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN country_count END) * 100.0 / total, 2) AS Upper_Middle,
  ROUND(SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN country_count END) * 100.0 / total, 2) AS Lower_Middle,
  ROUND(SUM(CASE WHEN `Income Group` = 'Low income' THEN country_count END) * 100.0 / total, 2) AS Low_Income,
  ROUND(SUM(CASE WHEN `Income Group` NOT IN ('High income','Upper middle income','Lower middle income','Low income') OR `Income Group` IS NULL THEN country_count END) * 100.0 / total, 2) AS Other
FROM 
  counts, totals
GROUP BY 
  `Region`
...
total AS (
  SELECT SUM(country_count) AS total FROM group_counts
)
SELECT 
  g.`Income Group`,
  g.country_count,
