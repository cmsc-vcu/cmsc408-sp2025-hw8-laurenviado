...
execute_ddl(cnx,"""
-- Fix Venezuela's missing Income Group
UPDATE wdi_country
...
run_sql_and_return_html(cnx,"""
-- Show verification with 3 columns
...
SELECT 
  c.`Region`,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'High income' THEN c.count ELSE 0 END), 1) AS High_Income,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Upper middle income' THEN c.count ELSE 0 END), 1) AS Upper_Middle,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Lower middle income' THEN c.count ELSE 0 END), 1) AS Lower_Middle,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Low income' THEN c.count ELSE 0 END), 1) AS Low_Income,
  ROUND(SUM(CASE WHEN c.`Income Group` NOT IN ('High income', 'Upper middle income', 'Lower middle income', 'Low income') OR c.`Income Group` IS NULL THEN c.count ELSE 0 END), 1) AS Other,
  ROUND(t.total, 1) AS Total
FROM counts c
