...
run_sql_and_return_html(cnx,"""
SELECT COUNT(DISTINCT d.`Country Code`) AS "Countries in 2020"
...
execute_ddl(cnx,"""
-- Fix Venezuela's missing income group classification
UPDATE wdi_country
...
run_sql_and_return_html(cnx,"""
-- Step 1: Get all valid combinations (region × income group)
...
-- Step 2: Get actual combinations from data
...
SELECT 
  ...
  c.`Income Group`, 
  c.country_count,
  ROUND(c.country_count * 100.0 / t.total_count, 2) AS percent_of_total
...
SELECT 
  c.`Region`,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'High income' THEN c.country_count ELSE 0 END) * 100.0 / t.total, 2) AS High_Income,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Upper middle income' THEN c.country_count ELSE 0 END) * 100.0 / t.total, 2) AS Upper_Middle,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Lower middle income' THEN c.country_count ELSE 0 END) * 100.0 / t.total, 2) AS Lower_Middle,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Low income' THEN c.country_count ELSE 0 END) * 100.0 / t.total, 2) AS Low_Income,
  ROUND(SUM(CASE WHEN c.`Income Group` NOT IN ('High income', 'Upper middle income', 'Lower middle income', 'Low income') THEN c.country_count ELSE 0 END) * 100.0 / t.total, 2) AS Other
FROM counts c, totals t
GROUP BY c.`Region`
ORDER BY c.`Region`;
""")
...
SELECT 
  ...
  g.country_count,
  t.total,
  ROUND(g.country_count * 100.0 / t.total, 2) AS percent_of_total
FROM 
  group_counts g, total_count t
ORDER BY g.country_count DESC;
""")
